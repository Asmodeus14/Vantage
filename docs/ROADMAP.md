...
| **Range-only Python projects are not dependency-scanned** | An exact version is required to query OSV. Python reads `poetry.lock` and `==` pins; a project declaring only `fastapi>=0.115` gets its dependencies listed but no advisories. Measured on this repository: 18 dependencies collected, 0 resolvable. | Parse `Pipfile.lock`, `uv.lock`, and `pip freeze`-style requirements. |
| **Go and Ruby get structural checks only** | Project detection is multi-language; correctness rules now cover JS/TS/React and Python. | The Python pack is the template — `applies(ctx)` already gates everything. |
...
3. **Streaming AI responses.** The provider already supports `stream()`; the
...
- **Python rule pack.** Four correctness and security rules, plus
  `requirements.txt`, `pyproject.toml` and `poetry.lock` parsing feeding the
  existing OSV client — which now queries per ecosystem instead of assuming npm.
